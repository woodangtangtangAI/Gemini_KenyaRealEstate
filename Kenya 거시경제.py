...
# 1. 1단계에서 수집한 일별 Raw 데이터 찾기 (대괄호 버그를 피하기 위해 안전한 방식 사용)
file_list = []
...
# 모든 csv 파일을 읽어서 리스트에 담은 뒤, 하나의 거대한 데이터프레임으로 결합
...
# 3. 거시경제 데이터 불러오기 및 결합
macro_file = os.path.join(base_path, "kenya_macro.csv") 

if not os.path.exists(macro_file):
    print(f"⚠️ 거시경제 파일({macro_file})을 찾을 수 없습니다. 부동산 데이터만으로 마스터를 생성합니다.")
    df_master = df_raw_clean
else:
    print("🔗 부동산 데이터와 거시경제 지표를 결합 중입니다...")
